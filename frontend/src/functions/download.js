...
async function downloadFileParts(
    _fileName,
    _fileArrayIndex,
    _fileArrayBegin,
    _fileArrayEnd,
    ...
    for (let i = 0; i < _fileArrayEnd - _fileArrayBegin; i += 500) {
        let endIndex =
            _fileArrayBegin + i + 500 < _fileArrayEnd ? i + 500 : _fileArrayEnd - _fileArrayBegin
        console.log('Downloading File Part: From ' + i + ' to ' + endIndex)
        setStatus('Downloading File Part: From ' + i + ' to ' + endIndex)
        let transaction = await contract.functions.getFileArray(
            _fileName,
            _fileArrayIndex,
            i,
            endIndex,
...
export async function downloadFile(
    ...
    if (fileArrayLength > 1) {
        ...
        await downloadFileParts(
            fileName,
            fileArrayLength - 1,
            (fileArrayLength - 1) * 10000,
            (fileArrayLength - 1) * 10000 + fileFinalPartLength,
            setDownloadedFileParts,
            setStatus,
        )
    } else {
...
export function encodeFile(downloadedFileParts, fileName, setStatus, setContentElement) {
